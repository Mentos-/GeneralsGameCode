...
static Real getHeightAroundPos(Real x, Real y)
{
	...
	terrainHeightMax = generals_max(terrainHeightMax, TheTerrainLogic->getGroundHeight(x+TERRAIN_SAMPLE_SIZE, y-TERRAIN_SAMPLE_SIZE));
	terrainHeightMax = generals_max(terrainHeightMax, TheTerrainLogic->getGroundHeight(x-TERRAIN_SAMPLE_SIZE, y-TERRAIN_SAMPLE_SIZE));
	terrainHeightMax = generals_max(terrainHeightMax, TheTerrainLogic->getGroundHeight(x+TERRAIN_SAMPLE_SIZE, y+TERRAIN_SAMPLE_SIZE));
	terrainHeightMax = generals_max(terrainHeightMax, TheTerrainLogic->getGroundHeight(x-TERRAIN_SAMPLE_SIZE, y+TERRAIN_SAMPLE_SIZE));
...
static void drawDrawableExtents( Drawable *draw, void *userData )
{
	...
	// Also draw a 2D screen-space bounding rectangle for this drawable to visualize on-screen extents
	{
		int minX = 0x7fffffff, minY = 0x7fffffff;
		int maxX = -0x7fffffff, maxY = -0x7fffffff;
		Bool any = FALSE;

		const GeometryInfo &ginfo = draw->getDrawableGeometryInfo();
		Coord3D basePos = *draw->getPosition();
