...
Int ThingTemplate::calcTimeToBuild( const Player* player) const
{
	...
	penaltyRate = generals_max(penaltyRate, TheGlobalData->m_MinLowEnergyProductionSpeed);	//bind so 0% does not dead stop you

	if( EnergyPercent < 1.0f )	//and make 99% look like 80% (eg) since most of the time you are down only a little
		penaltyRate = generals_min(penaltyRate, TheGlobalData->m_MaxLowEnergyProductionSpeed);
