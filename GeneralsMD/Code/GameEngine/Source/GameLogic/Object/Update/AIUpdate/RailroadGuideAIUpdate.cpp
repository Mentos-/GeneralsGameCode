...
//		  ai->aiIdle( CMD_FROM_AI );// this eliminates yadda by telling them to stop driving into me
	...
	{// we just run over debris, instead of shoving it around
		delta.scale( GENERALS_MIN(0.3f,m_pullInfo.speed * 0.66f) );
	}
	else
	{
		delta.scale( GENERALS_MIN(1.4f,m_pullInfo.speed  * 0.66f) );// the faster I go, the harder I slam!
	...
	heft.z = GENERALS_MAX(heft.z, TheTerrainLogic->getGroundHeight(heft.x, heft.y) + 2); // lift them off the ground
...
void RailroadBehavior::playImpactSound(Object *victim, const Coord3D *impactPosition)
{
	...
	vel = GENERALS_MIN(NORMAL_VEL_Z, GENERALS_MAX(0, vel));
	mass = GENERALS_MIN(NORMAL_MASS, GENERALS_MAX(0, mass));
...
void RailroadBehavior::FindPosByPathDistance( Coord3D *pos, const Real dist, const Real length, Bool setState )
{
	...
	else
	{
		...
		actualDistance = GENERALS_MAX(GENERALS_MIN(length,dist),0); // CLAMP TO NORMALIZE
	}
