...
{
	Extents.X = GENERALS_MAX(extents.X, 0.0f);
	Extents.Y = GENERALS_MAX(extents.Y, 0.0f);
	Extents.Z = GENERALS_MAX(extents.Z, 0.0f);
}
...
{
	float max = GENERALS_MAX(Extents.X, Extents.Y);
	max = GENERALS_MAX(max, Extents.Z);
	return max;
}

void Vector3SolidBoxRandomizer::Scale(float scale)
{
	scale = GENERALS_MAX(scale, 0.0f);
	Extents.X *= scale;
	Extents.Y *= scale;
	Extents.Z *= scale;
}
...
{
	Radius = GENERALS_MAX(radius, 0.0f);
}
...
{
	Extent = GENERALS_MAX(extent, 0.0f);
	Radius = GENERALS_MAX(radius, 0.0f);
}
...
{
	return GENERALS_MAX(Extent, Radius);
}
