...
void StreakLineClass::Set_Width(float width)
{
	...
	// alphas are).
	LineRenderer.Set_Width(GENERALS_MAX(width, 0.0f));
	StreakRenderer.Set_Width(GENERALS_MAX(width, 0.0f));
...
{
	MaxSubdivisionLevels = GENERALS_MIN(levels, MAX_SEGLINE_SUBDIV_LEVELS);
...
//   Set_Texture_Reduction_Factor(Calculate_Texture_Reduction_Factor(NormalizedScreenArea));
	...
	lvl = GENERALS_MIN(lvl, MaxSubdivisionLevels);
...
void StreakLineClass::Increment_LOD(void)
{
	...
	lvl = GENERALS_MIN(lvl+1,MaxSubdivisionLevels);
...
{
	lod = GENERALS_MAX(0, lod);
	lod = GENERALS_MIN(lod, (int)MaxSubdivisionLevels);
