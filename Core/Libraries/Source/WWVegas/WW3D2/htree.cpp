...
)
{
	...
	int num_anim_pivots = GENERALS_MIN( motion0->Get_Num_Pivots (), motion1->Get_Num_Pivots () );
...
)
{
	...
	for ( int anim_num = 0; anim_num < anim->Get_Num_Anims(); anim_num++ ) {
		num_anim_pivots = GENERALS_MIN( num_anim_pivots, anim->Peek_Motion( anim_num )->Get_Num_Pivots() );
	}
